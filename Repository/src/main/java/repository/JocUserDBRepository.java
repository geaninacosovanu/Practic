...
public class JocUserDBRepository implements IJocUserRepository {
    ...
    public JocUser save(JocUser entity) {
        try (PreparedStatement s = connection.prepareStatement("insert into JocUser(IdJoc, IdUser, Numere) VALUES (?,?,?)")) {
    ...
    @Override
    public List<JocUser> getDetalii(Integer idJoc) {
        List<JocUser> all = new ArrayList<>();
        try (PreparedStatement s = connection.prepareStatement("SELECT * FROM  JocUser  WHERE JocUser.IdJoc=?")) {
            s.setInt(1,idJoc);
            ResultSet resultSet = s.executeQuery();
            while (resultSet.next()) {
                all.add(new JocUser(resultSet.getString("IdUser"),resultSet.getInt("IdJoc"),resultSet.getString("Numere")));
            }
        }
        catch (SQLException e) {
